#!/usr/bin/env python3
"""Add 2 WHOIS + ASN lookup skills (R3 P1)."""
...
WHOIS_ASN_SKILLS: tuple = (
    SkillMeta(
        name="whois", risk="low",
        triggers=("whois","WHOIS 查询","域名注册人","registrar","域名所有者"),
        bins=("whois",), capabilities=(SHELL, NET, FS_R),
        install_brew=(("whois","whois"),),
        install_apt=(("whois","whois"),),
        description=(
            "Standard WHOIS client. Default usage: `whois target.com` returns "
            "registrar, creation/expiry dates, registrant org, name servers. "
            "Pair with `asnmap` to map registrant org → owned ASN → owned CIDR. "
            "Risk: low — read-only public WHOIS query."
        ),
        upstream_url="https://github.com/rfc1036/whois",
    ),
    SkillMeta(
        name="asnmap", risk="low",
        triggers=("asnmap","ASN 查询","ASN-IP 段","关联资产","asn map","cidr map"),
        bins=("asnmap",), capabilities=(SHELL, NET, FS_R),
        install_go=("github.com/projectdiscovery/asnmap/cmd/asnmap@latest",),
        install_brew=(("asnmap","asnmap"),),
        description=(
            "ProjectDiscovery ASN-to-CIDR + IP-to-ASN mapper. Default usage: "
            "`asnmap -d target.com` to map domain → ASN → CIDR ranges; "
            "`asnmap -asn AS12345` to dump CIDRs for a given ASN. Pair with "
            "infra_sharing ReconEvidence field for cross-target correlation. "
            "Risk: low — read-only public BGP data."
        ),
        upstream_url="https://github.com/projectdiscovery/asnmap",
    ),
)
...
def render_body(m: SkillMeta) -> str:
    return (f"\n# {m.name}\n\n{m.description}\n\n"
            ...
            f"Added by `scripts/add_whois_asn_skills.py` on {ATTRIBUTION_DATE} (R3 P1 S18).\n")
...
def main() -> int:
    ...
    for m in WHOIS_ASN_SKILLS: d = write_skill(m); print(f"     + {m.name} -> {d}"); n += 1
    print(f"OK  wrote {n}  whois-asn skills -> {DST_ROOT}"); return 0
