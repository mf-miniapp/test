...
| **Deep-attack orchestrator (`hack-deep` v4.0, 2026-06-11)** | A 4-layer × 15-wave attack-DAG coordinator (W0..W8 + W0.5/W0.6/W1.5/W1.5c/W2.5/W3.5) with Typed Task Envelopes, per-subdomain fan-out, drill-in reason classes, peer evidence attach, subagent-supervisor watchdog, and **v4.0 PARALLEL mode default** (override v3.2 serial; same wave 内 N 个 sessions_spawn 并发; SERIAL 仍 opt-in 供超大目标省 context). Cloned from `cyberstrike-deep`; co-resident, never replaces. See [`docs/hack-deep.md`](docs/hack-deep.md). |
| **hack-deep v4.0: W2.5 + W3.5 + Post-Exploitation Gate (2026-06-11)** | **W2.5 port attack plan** (R3): dynamic_fanout bucket=6, 读 `ReconEvidence.services` 生成 `PortAttackPlanEvidence`, per-port + vector 拆高复杂度 (1+ AttackVectorPlan per port). **W3.5 web crawl** (R4): dynamic_fanout bucket=4, 全 web service 并发爬虫 (katana + waybackurls + subjs + jsluice + feroxbuster), 产出 `WebCrawlEvidence` (hidden_paths / extracted_endpoints / auth_required_paths). **Post-Exploitation Gate** (R5): W4 `PenetrationEvidence.footholds` 空 → executor 跳过 W5/W6/W7, 写 `gate_skipped=True` 占位 evidence, 直跳 W8 reporting (未入侵 → 仅暴露面总结). 2 new evidence schemas (port-attack-plan-v1 + web-crawl-v1; EVIDENCE_SCHEMAS 16 → 18). 2 new waves (WAVES 13 → 15). Layer BREADTH 收纳 W3.5. |
| **Recon / Web coverage toolchain (40 skills)** | After the 2026-06-10 recon coverage gap fix, the recon + penetration + attack-surface specialists can actually invoke the tools their SOUL.md describes. **Full port scan**: `naabu` (SYN, 1-65535) + `nmap` (service + OS + NSE). **Directory / file brute**: `ffuf` (primary), `feroxbuster` (deep recursive), `gobuster` (DNS + vhost modes). **Vulnerability scan**: `sqlmap` (SQLi), `dalfox` (XSS), `nikto` (systemic, opt-in). **Manual payload templates** for SSRF / SSTI / LFI / XXE / IDOR live in `~/.opensquilla/payloads/manual-payloads.md` (nuclei explicitly excluded per user directive). **Wordlist**: SecLists `raft-medium-directories.txt` (~22k entries) under `~/.opensquilla/wordlists/`. Bootstrap with `python scripts/add_port_scan_skills.py && python scripts/add_dir_bust_skills.py && python scripts/add_vuln_scan_skills.py && python scripts/fetch_wordlists.py`. |
| **R3 P0+P1 expansion (19 more skills + W0.6 + DB/MQ two-phase)** | 2026-06-10 R3 adds 19 more recon / penetration skills under 8 new scripts. **P0 war-zone**: `s3scanner` + `cloud_enum` (cloud assets), `jenkins-cli` + `gitrob` (DevOps), `grafana-fingerprinter` + `prometheus-fingerprinter` (monitoring), `mongosh` + `redis-cli` + `elasticsearch-tools` + `rabbitmqadmin` + `kafkacat` (DB / MQ, ROE.aggressive-gated). **P1 breadth**: `whois` + `asnmap`, `cdncheck` + `wafw00f`, `tlsx` + `sslyze` (JA3/JA4 fingerprint), `gitleaks` + `trufflehog` (secret scan, 1000-commit cap). **W0.6 resource-checkpoint wave**: enumerates `~/.opensquilla/{skills,wordlists,payloads}` + `which <bin>` before W1 starts; fail-open (missing items warn but never block). **DNS depth**: `dnsx` SKILL.md runtime hint adds `axfr` / DMARC / SPF / DKIM TXT record parsing. **ServiceEntry.service precision**: W1 recon classifies ports into `mongodb` / `redis` / `kafka` / `jenkins` / etc. **W5 PrivescEvidence.current_access**: 7-key typed structure (os / net / proc / cron / env / creds / suid). 4 new wordlists: `devops-paths.txt` / `monitoring-paths.txt` / `cloud-buckets-prefixes.txt` / `db-ports.txt`. Total skill count after R3: **59** (40 R1 + 19 R3). Bootstrap with `python scripts/add_cloud_assets_skills.py && python scripts/add_devops_skills.py && python scripts/add_monitoring_skills.py && python scripts/add_db_mq_skills.py && python scripts/add_whois_asn_skills.py && python scripts/add_waf_cdn_skills.py && python scripts/add_ssl_tls_skills.py && python scripts/add_secret_scan_skills.py`. |
