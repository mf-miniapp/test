...
#   1. The grounding paragraph (so the subagent knows it is a subagent).
#   2. The Result Marker footer requirement (so the parent can correlate
#      the subagent's final reply with the HANDOFF envelope it sent and
#      detect partial / failed runs explicitly).
# The marker is parsed by attack_dispatch.envelope.RESULT_REGEX — keep the
# literal instruction text in lock-step with that regex.
#
# The text is sourced from a leaf module
# (``opensquilla.agents.subagent_contract``) so this file does NOT need to
# be on the import path of every consumer. Importing the leaf is safe
# because it has no transitive deps.
from opensquilla.agents.subagent_contract import (
    SUBAGENT_RESULT_MARKER_INSTRUCTION as _SUBAGENT_RESULT_MARKER_INSTRUCTION,
)
from opensquilla.agents.subagent_contract import build_subagent_system_prompt

# Public re-export of the marker instruction so existing callers and
# tests that import ``SUBAGENT_RESULT_MARKER_INSTRUCTION`` from
# ``tools.builtin.sessions`` keep working.
SUBAGENT_RESULT_MARKER_INSTRUCTION = _SUBAGENT_RESULT_MARKER_INSTRUCTION

_SUBAGENT_SYSTEM_PROMPT = build_subagent_system_prompt()
...
async def sessions_spawn(
    ...
    """Spawn a subagent session (v3.3, 2026-06-08).

    The ``auto_approve`` kwarg (2026-06-08, hack-deep v3.3, Issue 1)
    overrides the typed-envelope ``auto_approve`` field when set
    explicitly. Resolution order:

      1. The ``auto_approve`` kwarg (when not None).
      2. The envelope header's ``auto_approve=true|false`` field
         (when the task starts with a valid HANDOFF header).
      3. The parent's ``ToolContext.auto_approve_specialists`` flag
         (forwarded from the parent's executor).
      4. False (the conservative default; matches v3.2 behavior).

    When True, the spawned child session's tool context carries
    ``auto_approve_specialists=True`` so the shell approval
    pipeline returns a synthetic ``approval_denied`` envelope
    instead of raising ``UnsupportedSurfaceError``. This is the
    ...
    # envelope when the task starts with a HANDOFF header (the
    # hack-deep contract); the kwarg overrides when set.
    envelope_auto_approve: bool = False
    envelope: HandoffEnvelope | None = None
    try:
        envelope = parse_envelope(task)
        envelope_auto_approve = envelope.auto_approve
    except EnvelopeFormatError:
        # Task does not start with a HANDOFF header — older
        # caller's task format. ``envelope_auto_approve`` stays
        # False; the kwarg or the parent's ToolContext may still
        # set it.
        pass
    if auto_approve is None:
        ctx_for_flag = current_tool_context.get()
        if ctx_for_flag is not None and getattr(
            ctx_for_flag, "auto_approve_specialists", False
        ):
            effective_auto_approve = True
        else:
            effective_auto_approve = envelope_auto_approve
    else:
        effective_auto_approve = bool(auto_approve)
    # 2026-06-08 (Issue 1): stash the resolved flag on the current
    # tool context (if any) so the child session's spawn path
    # forwards it. The actual propagation happens in
    # ``build_subagent_route_envelope`` / the session manager's
    # create path; we tag the context so the next layer can read
    # it.
    if effective_auto_approve:
        ctx = current_tool_context.get()
        if ctx is not None:
            # Mark the parent context for the spawn path. We use
            # an attribute (not a class field) so the marker
            # survives the JSON round-trip via the session's
            # metadata blob. The gateway's spawn path reads this
            # and propagates it to the child.
            setattr(ctx, "_hack_deep_auto_approve_child", True)
    ...
    try:
        ...
        if allow_agents is not None:
            if resolved_agent_id == caller_agent_id:
                # Self-spawn: must be EXPLICITLY allowed. The previous
                # implementation skipped the allowlist check entirely for
                # self-spawn, which let coordinator agents accidentally spawn
                # themselves — the spawned sub-agent then ran with the
                # generic _SUBAGENT_SYSTEM_PROMPT (overriding the
                # coordinator's actual SOUL), tried to execute work that
                # wasn't in the sub-agent's tool allowlist, and failed.
                # 2026-06-05 fix: self-spawn requires self to be in
                # allow_agents, OR "*" to be present, OR allow_agents to
                # be empty (the `[] = self only` contract).
                if (
                    "*" not in allow_agents
                    and caller_agent_id not in allow_agents
                    and allow_agents != []
                ):
                    _log.warning(
                        "subagent.self_spawn_denied",
                        caller_agent_id=caller_agent_id,
                        allow_agents=allow_agents,
                    )
                    raise ToolError(
                        f"Self-spawn not allowed: '{caller_agent_id}' may not "
                        f"spawn itself unless its allow_agents contains itself, "
                        f"is empty (self-only), or contains '*' "
                        f"(allow_agents={allow_agents})"
                    )
            else:
                # Cross-agent spawn: target must be in allow_agents (or '*').
                # Per the schema contract, `allow_agents=[]` means
                # "self only" — so it does NOT permit cross-agent spawn
                # even if the list is non-empty (it's empty, target not
                # in it, no wildcard).
        ...
        envelope = build_subagent_route_envelope(
            ...
            parent_task_id=parent_task_id,
            spawn_depth=spawn_depth,
            # Cap the subagent's history to a small window so a long-running
            # recon/intel/attack-surface scan does not inherit the parent's
            # entire transcript. The parent passes the task in
            # ``grounded_task``; everything else is usually noise that
            # balloons the prompt to 80K+ tokens and trips the upstream
            # server-side 30s idle timeout. Operators can override per
            # agent via the ``agent.subagents.max_history_turns`` field in
            # the agent's config.
            extra_metadata={
                "route_max_history_turns": int(
                    os.environ.get(
                        "OPENSQUILLA_SUBAGENT_DEFAULT_HISTORY_TURNS",
                        "4",
                    )
                ),
            },
