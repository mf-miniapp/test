...
def _approval_denied_envelope(tool_name: str, *, reason: str, command: str = "") -> dict:
    """Build a synthetic ``approval_denied`` envelope (2026-06-08, Issue 1).

    Used by ``_check_exec_approval`` when the run is unattended
    AND the calling specialist has ``auto_approve_specialists=True``
    (set by hack-deep's parent LLM via
    ``sessions_spawn(..., auto_approve=True)``). The point is to
    let the W4 / W8 specialist's run CONTINUE — the offending
    tool returns a denied envelope that the specialist can react
    to (typically by skipping that probe), rather than crashing
    the whole wave on an ``UnsupportedSurfaceError``.

    Mirrors the inline shape used by the ``auto-deny`` mode
    (line ~1347). Centralized here so the auto-approve-specialists
    path and the auto-deny path produce consistent envelopes
    (downstream readers can detect the source via the
    ``auto_approve_specialists`` discriminator in the ``reason``
    field).
    """
    return {
        ...
        "reason": reason,
        "warning": (
            f"Tool '{tool_name}' denied: {reason}. The specialist "
            f"should react to this denial (typically by skipping "
            f"the probe) rather than retrying — the parent's "
            f"approval policy is final for this run."
        ),
        "message": reason,
    }
...
async def _check_exec_approval(
    ...
    if ctx is not None and ctx.interaction_mode is InteractionMode.UNATTENDED:
        # 2026-06-08 (hack-deep v3.3, Issue 1): differentiate
        # "unattended cron / channel run" (which has NO approval
        # surface — raising is correct) from "specialist child of a
        # hack-deep run with auto_approve_specialists=True" (which
        # has an approval surface; the parent just wants every
        # approval to auto-deny so the W4 / W8 specialist can keep
        # going). The latter returns a synthetic
        # ``approval_denied`` envelope; the former still raises.
        if getattr(ctx, "auto_approve_specialists", False):
            return _approval_denied_envelope(
                tool_name,
                reason=(
                    "auto_approve_specialists=True on this "
                    "specialist child; approval-gated tools auto-deny"
                ),
            )
