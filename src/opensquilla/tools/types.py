...
class ToolContext:
    ...
    # 2026-06-08 (hack-deep v3.3, Issue 1): when True, the shell
    # approval pipeline returns a synthetic ``approval_denied``
    # envelope for approval-gated tools instead of raising
    # ``UnsupportedSurfaceError``. Set by the parent LLM via the
    # ``sessions_spawn(..., auto_approve=True)`` kwarg or the typed
    # envelope's ``auto_approve=true`` field. Different from
    # ``interaction_mode=UNATTENDED`` (which is set by cron / channel
    # entry points that genuinely have no approval surface available);
    # a specialist child has an approval surface, it just wants
    # every approval to auto-deny so the parent's run continues.
