...
def _contains_context_payload_marker(value: str) -> bool:
    ...
    return any(
        ...
        for marker in (
            ...
            # engine/agent.py emits this exact phrase when the provider
            # returns no visible response after the input crossed the
            # context limit.  Without this marker the error leaks through
            # as a generic FAILED with a raw provider message; with it
            # the payload normalizes to ``provider_request_too_large``
            # and the user-facing terminal reply becomes the standard
            # "request is too large" line.
            "no visible response for a large input",
        )
    )
