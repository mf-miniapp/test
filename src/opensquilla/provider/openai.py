    ...
    async def _stream(
        ...
        if (caps and caps.supports_reasoning and cfg.thinking) or (
            ...
            if reasoning_format == "openrouter":
                ...
                # Only forward an explicit positive budget. The ``-1`` sentinel
                # in ``_THINKING_BUDGET_DEFAULT`` would otherwise be sent
                # verbatim, which llama-server (and possibly DashScope)
                # interprets as "unlimited" and logs as
                # ``reasoning-budget: activated, budget=2147483647 tokens``.
                # Letting the upstream pick its own default avoids both the
                # bogus log and a runaway token budget.
                raw_budget = cfg.thinking_budget_tokens
                if isinstance(raw_budget, int) and raw_budget > 0:
                    payload["thinking_budget"] = raw_budget
