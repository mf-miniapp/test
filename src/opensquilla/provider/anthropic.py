    ...
    async def _stream(
        ...
        if cfg.thinking:
            if _uses_adaptive_thinking(self._model):
                ...
                # Skip the budget field when the caller did not pick a
                # positive number — ``cfg.thinking_budget_tokens`` defaults
                # to 5000 in ``ChatConfig`` but a caller can leak the
                # ``_THINKING_BUDGET_DEFAULT = -1`` sentinel through. The
                # previous ``max(1, ...)`` clamped -1 to 1 token (almost no
                # thinking) which silently broke reasoning quality; omitting
                # the field lets Anthropic pick its own default instead.
                raw_budget = cfg.thinking_budget_tokens
