...
class ChatConfig(BaseModel):
    ...
    # llama.cpp context window. Ollama passes this straight through to the
    # underlying llama-server / llama.cpp process as ``num_ctx`` in the
    # request options. Default 131_072 (128 * 1024 = 128k) so a long
    # conversation does not have to truncate early when running against a
    # model whose Modelfile was built with a 2k/4k default. Set to 0 to
    # fall back to the model/Modelfile default.
    num_ctx: int = 131_072
