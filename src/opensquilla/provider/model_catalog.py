...
class ModelCatalog:
    ...
    def resolve_context_window(self, model_id: str, user_override: int = 0) -> int:
        """Resolve context window: user > catalog > static fallback > default.

        ``user_override`` is a positive-int override sourced from
        ``[llm].context_window_tokens`` in opensquilla.toml. When > 0 it
        short-circuits every other source so operators can pin a session
        to a budget different from the model's native window (e.g. clamp a
        1M model to 32k for cost reasons, or stretch a 16k model to 64k).

        Independence note: ``resolve_max_tokens`` calls this with the default
        ``user_override=0`` so a max_tokens override does not silently
        change the resolved context window.
        """
        if user_override > 0:
