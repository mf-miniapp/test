...
class AgentRegistry:
    ...
    async def create_agent(
        ...
        *,
        ...
        subagents: AgentSubagentDefaults | dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        ...
        entry = AgentEntryConfig(
            ...
            tier=tier or None,
            allowed_tiers=list(allowed_tiers) if allowed_tiers else None,
    ...
    async def update_agent(self, agent_id: str, **fields: Any) -> dict[str, Any]:
        ...
        if "subagents" in fields:
            updates["subagents"] = self._coerce_subagents(fields["subagents"])
    ...
    def _coerce_subagents(
        value: AgentSubagentDefaults | dict[str, Any] | None,
    ) -> AgentSubagentDefaults | None:
        """Accept either a model instance or a dict, return a model instance."""
        if value is None or isinstance(value, AgentSubagentDefaults):
            return value
        if isinstance(value, dict):
            return AgentSubagentDefaults.model_validate(value)
        raise TypeError(
            f"subagents must be AgentSubagentDefaults, dict, or None; got {type(value).__name__}"
        )
