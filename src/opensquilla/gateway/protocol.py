...
class PolicyInfo(BaseModel):
    ...
    # the chat UI re-issues chat.send up to N times with a synthetic
    # "please re-respond" prompt. ``0`` disables. The kinds list is
    # substring-matched against the terminal event's ``code`` / ``message``.
    webui_auto_retry_max_rounds: int = 2
    webui_auto_retry_on_kinds: list[str] = [
        "empty_response",
        "context_overflow",
        "timeout",
        "rate_limit",
