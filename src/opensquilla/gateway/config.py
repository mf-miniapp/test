...
class LlmProviderConfig(BaseSettings):
    ...
    # Explicit context-window override. 0 = auto-resolve from model catalog
    # (catalog → static fallback → 200_000); >0 = user-specified window in
    # tokens. Drives L1/L2 (provider-request proof) and L3/L4 (turn-boundary
    # compaction) budget math. Does NOT affect ChatConfig.num_ctx (Ollama
    # llama.cpp window) — that stays on its own 131_072 default and is tuned
    # server-side via OLLAMA_KV_CACHE_TYPE / OLLAMA_FLASH_ATTENTION.
    context_window_tokens: int = 0
    # L3 in-turn overflow compaction trigger, as a fraction of the resolved
    # context_window_tokens. Mirrors AgentConfig.context_overflow_threshold
    # (default 0.85). The bootstrap stage defends against out-of-range values
    # (<=0.0 or >1.0) by silently falling back to 0.85.
    context_overflow_threshold: float = 0.85
    # Token budget for the post-compaction step-trail overlay. When the
    # ``StepTrailRebuilderHook`` rebuilds the trail view from disk it
    # truncates to this many tokens (4 chars/token) so a long run does
    # not blow up the next turn's request budget. Mirrors
    # ``AgentConfig.request_context_prompt_max_tokens``; the bootstrap
    # stage wires the value into the per-turn config.
    request_context_prompt_max_tokens: int = 2000
...
class AgentEntryConfig(BaseModel):
    ...
    # waves. Operators should set this to a small number (4-8) for any
    # long-running orchestrator agent. The value is forwarded into the
    # spawn envelope as ``route_max_history_turns`` so the turn runner
    # honours it.
    max_history_turns: int = Field(default=0, ge=0, le=200)
...
class SubagentSupervisorConfig(BaseSettings):
    """Watchdog for stuck or unannounced subagent completions.

    Off by default. Set ``enabled=True`` to install a recurring cron that
    scans every open spawn group and replays the announce for any child
    whose terminal event was dropped on the floor (e.g. because the
    primary path's dedup set had already been tripped by an earlier
    sibling, see the 2026-06-05 hack-deep incident). The supervisor
    reuses ``_send_parent_wake`` so dedup state is shared with the
    primary announce path.
    """

    model_config = SettingsConfigDict(
        env_prefix="OPENSQUILLA_SUBAGENT_SUPERVISOR_",
        extra="forbid",
    )

    enabled: bool = False
    interval_seconds: int = Field(default=60, ge=5, le=3600)
    stuck_threshold_seconds: float = Field(default=600.0, ge=30.0)
...
class GatewayConfig(BaseSettings):
    ...
    subagent_supervisor: SubagentSupervisorConfig = Field(
        default_factory=SubagentSupervisorConfig
    )
    ...
    # Maximum number of WebUI-side auto-retries when a turn ends with a
    # recoverable provider error (empty response, context overflow, timeout,
    # rate limit, 5xx). Each retry re-issues ``chat.send`` with a synthetic
    # "please re-respond" prompt on the same session, giving the engine a
    # fresh provider-retry budget. ``0`` disables auto-retry entirely.
    webui_auto_retry_max_rounds: int = Field(default=2, ge=0, le=8)
    # Error-kind allow-list for the WebUI auto-retry layer. Substring-matched
    # (case-insensitive) against the terminal event's ``code`` and
    # ``message`` payload. Default covers the four kinds the operator
    # confirmed on 2026-06-10 (empty / overflow / timeout / 429 / 5xx).
    webui_auto_retry_on_kinds: tuple[str, ...] = (
        "empty_response",
        "context_overflow",
        "timeout",
        "rate_limit",
