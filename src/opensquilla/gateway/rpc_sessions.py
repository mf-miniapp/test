...
async def _handle_sessions_send(params: dict | None, ctx: RpcContext) -> dict:
    ...
    # Cap conversation history turns for parent (non-subagent) turns so
    # orchestrator agents (e.g. SQUILLA) do not balloon to 80K+ tokens
    # after a few waves of subagent completions. Subagents get their own
    # cap (smaller, default 4) via ``sessions_spawn``; parents can use a
    # larger window since they need to see the synthesized subagent
    # outputs. Operators can override per request via
    # ``OPENSQUILLA_PARENT_DEFAULT_HISTORY_TURNS`` (default 8).
    if "route_max_history_turns" not in route_envelope.metadata:
        try:
            default_parent = int(
                os.environ.get(
                    "OPENSQUILLA_PARENT_DEFAULT_HISTORY_TURNS",
                    "8",
                )
            )
        except ValueError:
            default_parent = 8
        if default_parent > 0:
            route_envelope.metadata["route_max_history_turns"] = default_parent
