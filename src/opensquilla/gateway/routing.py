...
def build_subagent_route_envelope(
    *,
    ...
    if extra_metadata:
        # Caller-supplied metadata is merged AFTER the standard fields so
        # that callers can override anything (e.g. force a small
        # max_history_turns on long-running recon subagents whose prompt
        # would otherwise balloon to 80K+ tokens and trip the upstream
        # idle timeout).
