...
class BackgroundCompletionManager:
    ...
    def __init__(
        ...
        *,
        ...
        # group_id -> set of child_session_keys whose wake is currently in
        # flight (scheduled but not yet acked). Per-child dedup lets a
        # follow-up wake fire if it carries children not yet in flight.
        self._wake_groups: dict[str, set[str]] = {}
    ...
    async def emit_waiting(
        ...
        *,
        ...
        async with self._state_lock:
            if group_id in self._waiting_groups or self._wake_groups.get(group_id):
                return
    ...
    async def send_parent_wake(
        ...
        *,
        ...
        Per-child dedup: a wake is dropped only if every child_session_key
        it would deliver is already in the in-flight set for this group.
        The pre-fix boolean set marked the group as "in flight" after the
        ...
        async with self._state_lock:
            if self._closing:
                return
            in_flight = self._wake_groups.get(group_id, set())
            if new_children and new_children.issubset(in_flight):
                return
            self._wake_groups[group_id] = in_flight | new_children
