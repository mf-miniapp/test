...
const ChatView = (() => {
  ...
  // WebUI auto-retry (2026-06-10). When a turn ends with a recoverable
  // provider error (empty response, context overflow, transport timeout,
  // rate limit, 5xx), the chat UI re-issues chat.send up to N times with
  // a synthetic "please re-respond" prompt on the same session. The
  // engine then starts a brand-new turn with a fresh provider-retry
  // budget. Per-session counter so multi-tab operators don't collide.
  let _autoRetryRounds = new Map();   // sessionKey -> rounds already burned
  let _autoRetryInFlight = false;     // true while the synthetic chat.send is in flight
  let _autoRetryAbortFlag = false;    // true if the user clicked Cancel
  let _autoRetryIndicatorEl = null;   // DOM element of the visible indicator bubble
  let _autoRetryMaxRounds = 2;        // pushed from server via _applyAutoRetryPolicy
  let _autoRetryOnKinds = [           // substring match against code+message
    'empty_response', 'context_overflow', 'timeout', 'rate_limit', '5xx',
  ];
  ...
  function render(el) {
    ...
    _applyAutoRetryPolicy(_rpc?.policy || {});
  ...
  function _subscribeRpcEvents() {
    ...
    _unsubs.push(_rpc.on('*', (rawEvent, rawPayload, rawMeta = {}) => {
      ...
      if (event.endsWith('.done') || event === 'chat.done') {
        ...
        // Auto-retry (2026-06-10): natural completion (success or abort)
        // clears the per-session counter and indicator so the next user
        // message starts from a clean slate.
        if (_sessionKey) _autoRetryRounds.delete(_sessionKey);
        _autoRetryInFlight = false;
        _autoRetryAbortFlag = false;
        _removeAutoRetryIndicator();
      } else if (event.endsWith('.error')) {
        _endStreaming();
        // Auto-retry (2026-06-10): when a turn ends with a recoverable
        // provider error, re-issue the send RPC with a synthetic
        // "please re-respond" prompt so the engine gets a fresh
        // provider-retry budget. The operator sees a grey indicator
        // bubble with a Cancel button instead of the raw error.
        if (_shouldAutoRetry(payload)) {
          _scheduleAutoRetry(payload);
        } else {
    ...
    _unsubs.push(_rpc.on('_hello', (hello) => {
      ...
      _applyAutoRetryPolicy(hello?.policy || {});
    }));
  ...
  async function _onSend() {
    ...
    // Auto-retry (2026-06-10): user-typed send is a stronger signal than
    // any synthetic retry. Clear the per-session counter so the next
    // transient error starts from round 0.
    if (_sessionKey) _autoRetryRounds.delete(_sessionKey);
    _autoRetryInFlight = false;
    _autoRetryAbortFlag = false;
    _removeAutoRetryIndicator();
    ...
    const now = new Date().toISOString();
    let userText = text;
    let providerText = text || 'Describe these attachments';
    ...
    // ── @-mention routing ───────────────────────────────────────────────
    // If the user message starts with `@<agent_id>`, dispatch directly to
    // that agent's session. This bypasses SQUILLA (main) so the LLM that
    // receives the message is the *mentioned* agent's, not the entry
    // coordinator. Strips the @agent_id prefix from the message body.
    const _mentionMatch = /^\s*@([a-zA-Z0-9_-]+)\s+([\s\S]*?)\s*$/.exec(providerText);
    if (_mentionMatch) {
      const mentionedAgent = _normalizeAgentId(_mentionMatch[1]);
      const bodyAfterMention = _mentionMatch[2];
      // Reject "main" — that's the routing layer, not a target.
      if (mentionedAgent && mentionedAgent !== 'main') {
        // Generate a new webchat session key bound to the mentioned agent.
        // The backend's chat.send auto-creates the session on demand.
        const newSessionKey = _webchatSessionKey(
          mentionedAgent,
          Math.random().toString(36).slice(2, 10)
        );
        // Build params with the new session key + stripped message body.
        // We rebuild the user-text to *not* show the @-prefix in the
        // message bubble, since the routing is implicit.
        providerText = bodyAfterMention;
        userText = bodyAfterMention;
        // Re-render the user bubble without the @-prefix.
        userBody.innerHTML = _esc(userText);
        // Switch the active session so subsequent messages continue there.
        if (newSessionKey !== _sessionKey) {
    ...
    _rpc.call('chat.send', params).then((res) => {
      ...
      _endStreaming();
      // Auto-retry (2026-06-10): if the RPC failure matches a
      // recoverable error kind, try once more instead of surfacing
      // "Send failed: …" to the operator.
      if (_shouldAutoRetry({ code: 'rpc_error', message: err && err.message ? err.message : String(err) })) {
        _scheduleAutoRetry({ code: 'rpc_error', message: err && err.message ? err.message : String(err) });
      } else {
        _addMessage('error', 'Send failed: ' + (err && err.message ? err.message : err));
      }
    });
  }
  ...
  function _applyAutoRetryPolicy(policy) {
    const rawMax = policy && policy.webui_auto_retry_max_rounds;
    if (typeof rawMax === 'number' && Number.isFinite(rawMax) && rawMax >= 0) {
      _autoRetryMaxRounds = Math.min(8, Math.floor(rawMax));
    } else {
      _autoRetryMaxRounds = 2;
    }
    const rawOn = policy && Array.isArray(policy.webui_auto_retry_on_kinds)
      ? policy.webui_auto_retry_on_kinds
      : null;
    if (rawOn && rawOn.length) {
      _autoRetryOnKinds = rawOn.map(s => String(s).toLowerCase());
    } else {
      _autoRetryOnKinds = ['empty_response', 'context_overflow', 'timeout', 'rate_limit', '5xx'];
    }
  }

  // Pure-ish decision helper. Returns true when the WebUI should
  // automatically re-issue chat.send for a terminal error event.
  // Caller supplies ``payload`` (the terminal session.event.error frame)
  // and we consult the policy pushed by _applyAutoRetryPolicy. The
  // function is intentionally cheap and side-effect free so it is safe
  // to call from both the .error branch and the chat.send RPC .catch.
  function _shouldAutoRetry(payload) {
    if (!_autoRetryMaxRounds) return false;
    if (_autoRetryInFlight) return false;
    if (_aborted) return false;
    const sessionKey = _sessionKey || '';
    if (!sessionKey) return false;
    const used = _autoRetryRounds.get(sessionKey) || 0;
    if (used >= _autoRetryMaxRounds) return false;
    const code = String((payload && payload.code) || '').toLowerCase();
    const msg = String((payload && payload.message) || '').toLowerCase();
    const haystack = code + ' ' + msg;
    for (let i = 0; i < _autoRetryOnKinds.length; i++) {
      if (haystack.indexOf(_autoRetryOnKinds[i]) !== -1) return true;
    }
    return false;
  }

  // Issue a synthetic user message that re-enters the engine with a
  // fresh provider-retry budget. The visible bubble makes the retry
  // transparent and exposes a Cancel button.
  function _scheduleAutoRetry(payload) {
    if (!_sessionKey) return;
    const sessionKey = _sessionKey;
    const used = (_autoRetryRounds.get(sessionKey) || 0) + 1;
    _autoRetryRounds.set(sessionKey, used);
    _autoRetryInFlight = true;
    _autoRetryAbortFlag = false;

    const synthText = '[Auto-retry ' + used + '/' + _autoRetryMaxRounds +
      '] Please re-respond to my previous message.';
    _addAutoRetryIndicator(used, _autoRetryMaxRounds);

    const params = { message: synthText, sessionKey };
    _rpc.call('chat.send', params).then((res) => {
      if (res && res.sessionKey && res.sessionKey !== _sessionKey) {
        _persistSession(res.sessionKey);
      }
    }).catch((err) => {
      _autoRetryInFlight = false;
      _removeAutoRetryIndicator();
      const errMsg = (err && err.message) ? err.message : String(err);
      _addMessage('error', 'Auto-retry send failed: ' + errMsg);
    });
  }

  // User clicked "Cancel" on the indicator bubble: stop the current
  // retry round but leave the counter alone (next failure will
  // surface normally). The in-flight chat.send RPC is left to complete
  // — its response will be ignored because _autoRetryAbortFlag is set
  // and the engine has already short-circuited the engine-level retry.
  function _cancelAutoRetry() {
    _autoRetryAbortFlag = true;
    _autoRetryInFlight = false;
    _removeAutoRetryIndicator();
    if (_sessionKey) _autoRetryRounds.delete(_sessionKey);
  }

  function _addAutoRetryIndicator(used, max) {
    _removeAutoRetryIndicator();
    if (!_thread) return;
    const el = document.createElement('div');
    el.className = 'msg msg-auto-retry';
    el.setAttribute('data-auto-retry', used + '/' + max);
    el.innerHTML =
      '<div class="msg-body">⟳ Auto-retrying (' + used + '/' + max + ')…' +
      '<button type="button" class="auto-retry-cancel" ' +
      'aria-label="Cancel auto-retry">Cancel</button></div>';
    const cancelBtn = el.querySelector('.auto-retry-cancel');
    if (cancelBtn) {
      cancelBtn.addEventListener('click', () => _cancelAutoRetry());
    }
    _thread.appendChild(el);
    _autoRetryIndicatorEl = el;
    if (_autoScroll) {
      _thread.scrollTop = _thread.scrollHeight;
    }
  }

  function _removeAutoRetryIndicator() {
    if (_autoRetryIndicatorEl && _autoRetryIndicatorEl.parentNode) {
      _autoRetryIndicatorEl.parentNode.removeChild(_autoRetryIndicatorEl);
    }
    _autoRetryIndicatorEl = null;
  }
  ...
  function _onStop(source = 'webui_stop_button') {
    ...
    // Auto-retry (2026-06-10): user-initiated abort cancels any pending
    // auto-retry and resets the per-session counter.
    _autoRetryAbortFlag = true;
    _autoRetryInFlight = false;
    _removeAutoRetryIndicator();
    if (_sessionKey) _autoRetryRounds.delete(_sessionKey);
