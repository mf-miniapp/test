...
class _ProviderRetryPolicy:
    ...
    def from_provider_budget(
        ...
        *,
        ...
        # 2026-06-09 (Issue 8 — empty-response auto-skip). The
        # empty-response budget used to be 1: the agent would retry
        # once and then bail out with a terminal_error that blocked
        # the parent (hack-deep) waiting on the failing subagent.
        # The 51ifind.com 2026-06-09 stress run showed that an
        # empty response is OFTEN a transient provider hiccup
        # (proxy drop, transient 5xx, cold cache) that resolves on
        # the 2nd or 3rd retry. The 3-attempt budget is the
        # sweet spot between giving up too early (1 retry misses
        # real recovery) and retrying forever (wasting budget on
        # truly broken models). After 3 empty responses the
        # subagent supervisor (see subagent_supervisor.py) auto-
        # skips the failing child and writes a "AUTO-SKIP" audit
        # marker so the parent moves to the next fan-out sub-track
        ...
        return cls(
            ...
            provider_failure_budgets={
                ProviderFailureKind.EMPTY_RESPONSE: empty_response_budget,
            },
        )
...
class Agent:
    ...
    def _context_overflow_error(self) -> ErrorEvent:
        ...
        if reason == "memory_flush_timeout_before_compaction":
            return ErrorEvent(
                message=(
                    ...
                    "memory flush timed out. The provider request was NOT "
                    "compacted; raise max_flush_timeout_seconds, disable "
                    "flush_compaction_requires_safe_receipt, or shrink the "
        ...
        if reason == "memory_flush_degraded_before_compaction":
            return ErrorEvent(
                message=(
                    "Context compaction could not run because the pre-compaction "
                    "memory flush did not produce a verified summary. The "
                    "provider request was NOT compacted. Either retry (the "
                    "flush may complete on the next cycle) or disable "
                    "flush_compaction_requires_safe_receipt to allow "
                    "degraded-flush compaction."
                ),
        ...
        if reason == "empty_summary_rejected":
            return ErrorEvent(
                message=(
                    "Context compaction produced no replacement summary "
                    "(compactor rejected the empty summary). The provider "
                    "request was NOT reduced; consider a smaller model or "
                    "manually trimming the conversation."
                ),
        ...
        if reason == "compaction_failed":
            return ErrorEvent(
                message=(
                    ...
                    "could be retried. The compaction step raised an "
                    "exception; check the compactor logs for the underlying "
                    "error."
                ),
        ...
        if reason == "compaction_not_smaller":
            return ErrorEvent(
                message=(
                    "Context compaction ran but did not reduce the provider "
                    "request. The summary is larger than (or as large as) the "
                    "messages it replaced. The simple-fallback path "
                    "(drop-oldest-messages / drop-largest-tool-result) was "
                    "also tried and was not enough. Try a smaller model or "
                    "shrink the prompt manually."
                ),
                code="compaction_not_smaller",
            )
        if reason == "simple_fallback_not_smaller":
            # 2026-06-09 (Issue 9): the simple-fallback path ran
            # (drop-oldest / drop-largest-tool-result) but the result
            # was not smaller than the input. Distinct from
            # ``compaction_not_smaller`` so operators can tell which
            # path failed.
            return ErrorEvent(
                message=(
                    "Context compaction AND the simple-fallback path "
                    "(drop-oldest-messages / drop-largest-tool-result) "
                    "both failed to reduce the provider request. The "
                    "conversation is too long for the configured "
                    "context window. Try a smaller model, raise "
                    "context_window_tokens, or split the work into "
                    "multiple turns."
                ),
        ...
        if reason == "provider_recent_tail_too_large":
            return ErrorEvent(
                message=(
                    "Context compaction ran, but the most recent turn "
                    "(user prompt + tool_result blocks) is still larger "
                    "than the configured context window. The simple-"
                    "fallback path (drop-oldest-messages / drop-largest-"
                    "tool-result) was tried and was not enough. Try a "
                    "larger context_window_tokens, a smaller model, or "
                    "trim the prompt manually."
                ),
                code="provider_request_too_large",
            )
        if reason == "provider_request_budget_exhausted":
            return ErrorEvent(
                message=(
                    "The request is too large for the provider context "
                    "window. OpenSquilla attempted context compaction and "
                    "the simple-fallback payload reduction (drop oldest "
                    "messages / drop largest tool_result), but neither "
                    "reduced the request below the budget. Try a larger "
                    "context_window_tokens, a smaller model, or trim the "
                    "prompt manually."
                ),
                code="provider_request_too_large",
            )
        return ErrorEvent(
            message=(
                "Context overflow persists after compaction and the "
                "simple-fallback path. Try a larger context_window_tokens, "
                "a smaller model, or trim the prompt manually."
            ),
                ...
                while _retry_attempt <= _fallback.max_retries:
                    ...
                    if not _got_error and attempt_classification.kind != _ProviderAttemptKind.OK:
                        ...
                        if large_context_invalid:
                            # First try local context compaction: an empty /
                            # reasoning-only response on a large input is
                            # usually the provider failing to honor a too-big
                            # request. Shrink the request, then retry.
                            #
                            # Only skip the retry-tail-to-compact loop when
                            # compaction is refused because the recent tail
                            # is already larger than the budget — no amount
                            # of summary work can save that. In that case
                            ...
                            if overflow_retries < self.config.max_overflow_retries:
                                ...
                                yield WarningEvent(
                                    ...
                                    message=(
                                        "Provider returned no visible response for a large "
                                        "input; compacting older context before retrying."
                                    ),
                                )
                                overflow_outcome = await self._check_context_overflow(
                                    turn_messages,
                                    max(
                                        int(iter_input_tokens or 0),
                                        self._estimate_live_request_tokens(
                                            request_messages,
                                            tools=provider_tools_for_call,
                                            config=call_chat_cfg,
                                        ),
                                ...
                                if (
                                    overflow_outcome is not None
                                    and overflow_outcome.compacted
                                    and self._last_compaction_refusal_reason
                                    ...
                                    if self._provider_request_is_smaller(
                                        request_messages,
                                        ...
                                        yield WarningEvent(
                                            ...
                                            message=(
                                                "Context compacted; retrying the provider request."
                                            ),
                                        )
                                        ...
                                        _call_attempt += 1
                                        continue
                                    # Compaction didn't shrink the request —
                                    # remember the reason and fall through to
    ...
    async def _check_context_overflow(
        ...
        *,
        ...
        if self.config.flush_enabled:
            ...
            if not flush_receipt_allows_destructive_compaction(receipt):
                ...
                if self.config.flush_compaction_requires_safe_receipt:
                    ...
                    # 2026-06-09 (Issue 9): the flush refused to
                    # give a safe receipt, so the LLM-driven
                    # compactor would also refuse. Skip it and
                    # try the simple fallback (drop oldest /
                    # truncate outsized tool_result) which does
                    # NOT need a flush receipt. If the simple
                    # fallback produces a smaller request, the
                    # caller retries the provider with the
                    # compacted list; if not, we still return
                    # None (the original flush-refusal reason
                    # is preserved so the error message is
                    # honest).
                    simple_outcome = self._simple_fallback_compact(
                        messages,
                        context_window_tokens=window_tokens,
                        estimated_context_tokens=estimated_context_tokens,
                        compaction_id=compaction_id,
                    )
                    if simple_outcome is not None:
        ...
        try:
            ...
            # 2026-06-09 (Issue 9): main compactor raised. Try
            # the simple fallback (drop oldest / truncate
            # outsized tool_result) before giving up. The
            # simple fallback is side-effect-free (no flush, no
            # LLM call) so it can run safely after the main
            # compactor already failed. If it produces a
            # smaller request, return that; otherwise the
            # simple-fallback helper sets
            # ``_last_compaction_refusal_reason =
            # "simple_fallback_not_smaller"`` and we return
            # None (the caller will retry the provider call
            # with the compacted list if the simple fallback
            # succeeded, or surface an honest error if not).
            simple_outcome = self._simple_fallback_compact(
                messages,
                context_window_tokens=window_tokens,
                estimated_context_tokens=estimated_context_tokens,
        ...
        if result.removed_count == 0 and not result.summary and has_structured_content:
            ...
            # 2026-06-09 (Issue 9): the LLM compactor returned a
            # no-op (``removed_count=0`` on structured content).
            # The structured-content noop means the compactor
            # chose not to summarise. Try the simple fallback
            # (drop oldest / truncate outsized tool_result)
            # before giving up.
            simple_outcome = self._simple_fallback_compact(
                messages,
                context_window_tokens=window_tokens,
    ...
    # 2026-06-09 (Issue 9 — compaction did not run, error message
    # lies). When the LLM-driven compactor refuses / fails / runs
    # but does not reduce the request, the legacy code surfaced
    # the "after automatic context compaction and payload
    # reduction" message even though compaction DID NOT actually
    # run. The 51ifind.com 2026-06-09 stress test exposed the gap:
    # the user set a large context_window_tokens, the runtime
    # claimed it had done compaction, but the compaction step
    # returned ``removed_count=0`` / raised / the flush refused
    # to give a safe receipt. The legacy fallback was to just
    # throw the user-facing error.
    #
    # The fix adds a deterministic, lightweight
    # ``_simple_fallback_compact`` that runs AFTER the main
    # compactor's failure paths. It does NOT need a flush receipt
    # and does NOT need the LLM-driven compactor — it just drops
    # the oldest non-system messages and the largest tool_result
    # blocks until the request fits the configured window. When
    # the simple fallback produces a smaller request, the
    # function returns a ``CompactionOutcome`` with
    # ``compacted=True`` so the caller can retry the provider
    # call. When even the simple fallback can't reduce the
    # request, it sets
    # ``_last_compaction_refusal_reason = "simple_fallback_not_smaller"``
    # so the error message can honestly say "compaction AND the
    # simple-fallback path both failed to reduce the request."

    _SIMPLE_FALLBACK_DROP_OLDEST_FRACTION: float = 0.25
    """Fraction of (non-system) messages to drop in the first
    simple-fallback pass. With 100 messages, drop 25; with 20,
    drop 5; with 2, drop 0 (we keep at least the last 2
    user/assistant turns)."""

    _SIMPLE_FALLBACK_TRUNCATE_KEEP_FRACTION: float = 0.5
    """For each kept tool_result that is larger than the
    average message size, truncate to this fraction of its
    original content. 0.5 keeps the first half and drops the
    second."""

    def _simple_fallback_compact(
        self,
        messages: list[Message],
        *,
        ...
        """Lightweight, deterministic fallback compaction (Issue 9).

        Used when the LLM-driven compactor refuses / fails / runs
        but does not reduce the request. The simple fallback:

          1. Drop the oldest ``_SIMPLE_FALLBACK_DROP_OLDEST_FRACTION``
             fraction of non-system messages, keeping at minimum
             the last 2 user / assistant messages (we never drop
             the most recent turn).
          2. For each remaining tool_result block whose
             serialized content is larger than the median
             message size, truncate to
             ``_SIMPLE_FALLBACK_TRUNCATE_KEEP_FRACTION`` of its
             original length and append a ``[truncated]`` marker.
          3. Recompute the estimated token count. If the new
             size is < the configured context window, return
             ``CompactionOutcome(compacted=True)``. If still too
             large OR not smaller than the input, return
             ``None`` and set
             ``_last_compaction_refusal_reason = "simple_fallback_not_smaller"``.

        The simple fallback is intentionally side-effect free
        (no flush, no LLM call, no I/O) so it can run after the
        main compactor's failure paths without making the
        overflow any worse. When the simple fallback succeeds,
        the caller retries the provider call with the trimmed
        message list. When it fails, the caller surfaces an
        honest "compaction AND simple-fallback both failed"
        error message to the operator.
        """
        if not messages:
            self._last_compaction_refusal_reason = "simple_fallback_not_smaller"
            return None

        # Step 1: drop oldest non-system messages.
        # "System" messages (role=system or role=developer) are
        # always kept — they're the agent's grounding + skill
        # context. We only drop the oldest
        # user/assistant/tool messages.
        n_total = len(messages)
        n_drop = max(0, int(n_total * self._SIMPLE_FALLBACK_DROP_OLDEST_FRACTION))
        # Keep at minimum the last 4 messages (last user turn +
        # last assistant turn + room for tool calls).
        n_drop = min(n_drop, max(0, n_total - 4))
        if n_drop > 0:
            messages = list(messages[n_drop:])

        # Step 2: truncate oversized tool_result blocks.
        # We compute the median message size first so we only
        # touch the outsized blocks.
        sizes = [
        ...
        truncate_target = max(
            512,  # never go below 512 chars; that's still useful
            int(median_size * self._SIMPLE_FALLBACK_TRUNCATE_KEEP_FRACTION),
        )
        ...
        for m in messages:
            ...
            if isinstance(content, list):
                ...
                for block in content:
                    if (
                        hasattr(block, "tool_use_id")
                        and hasattr(block, "content")
                        and isinstance(getattr(block, "content", None), str)
                        and len(getattr(block, "content")) > truncate_target
                    ):
                        original = getattr(block, "content")
                        kept = original[:truncate_target]
                ...
                new_messages.append(
                    Message(role=m.role, content=new_blocks)
                )
            elif isinstance(content, str) and len(content) > truncate_target:
                kept = content[:truncate_target] + "\n\n[truncated]"
                new_messages.append(Message(role=m.role, content=kept))
                any_truncated = True
            else:
                new_messages.append(m)

        if not any_truncated and n_drop == 0:
            # Nothing to drop, nothing to truncate — simple
            # fallback can't help.
            logger.info(
                "context_overflow.simple_fallback.no_op",
                session_key=self._session_key,
                messages=n_total,
        ...
        # Step 3: estimate the new size and compare.
        new_size_chars = sum(
            len(m.content) if isinstance(m.content, str)
            else sum(
                len(getattr(b, "content", "") or "")
        ...
        # binary "smaller than before" check.
        new_size_tokens = max(1, new_size_chars // 4)
        if new_size_tokens >= estimated_context_tokens:
            logger.warning(
                "context_overflow.simple_fallback_not_smaller",
                session_key=self._session_key,
                tokens_before=estimated_context_tokens,
                tokens_after=new_size_tokens,
        ...
        logger.info(
            "context_overflow.simple_fallback_succeeded",
            session_key=self._session_key,
            tokens_before=estimated_context_tokens,
            tokens_after=new_size_tokens,
            context_window=context_window_tokens,
            messages_dropped=n_drop,
            blocks_truncated=any_truncated,
        )
        if self._session_key:
            try:
                notify_compaction(
                    ...
                    source="automatic",
                    phase="agent_inline_overflow",
                    status="simple_fallback_applied",
                    tokens_before=estimated_context_tokens,
                    tokens_after=new_size_tokens,
        ...
        return CompactionOutcome(
            ...
            compacted=True,
            removed_count=n_drop,
            # No LLM-generated summary; the simple fallback just
            # shrinks the message list. The downstream provider
            # call sees the trimmed list and (hopefully) fits.
            summary="",
        )
