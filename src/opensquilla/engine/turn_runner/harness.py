...
class _TurnRunnerModelCatalogAdapter(ModelCatalogPort):
    ...
    def lookup(self, model_id: str) -> _ResolvedCatalog:
        ...
        user_context_window = getattr(llm_cfg, "context_window_tokens", 0)
        if runner._model_catalog is not None:
            ...
            context_window = runner._model_catalog.resolve_context_window(
                model_id, user_override=user_context_window
            )
            ...
            # honor the user override even when no catalog is available
            context_window = (
                user_context_window if user_context_window > 0 else 200_000
            )
            capabilities = None
