...
# Sentinel default for L3 in-turn overflow compaction threshold.
# Matches AgentConfig.context_overflow_threshold (engine/types.py:308).
_OVERFLOW_THRESHOLD_DEFAULT = 0.85


def _resolve_overflow_threshold(turn: Any) -> float:
    """Read [llm].context_overflow_threshold with a safe fallback.

    The field is intentionally unconstrained at the config layer (mirroring
    ``max_tokens`` style) so operators can experiment. A bad value here is
    dangerous, though — ``threshold <= 0`` makes the L3 trigger fire on
    every turn (engine/agent.py:4166-4173 compares ``estimated > threshold *
    window``), and ``threshold > 1.0`` makes it never fire. Silently fall
    back to the AgentConfig default in those cases. NaN is caught
    explicitly because every comparison against NaN returns False, so it
    would otherwise pass the ``<= 0.0 or > 1.0`` guard.
    """
    llm_cfg = getattr(getattr(turn, "config", None), "llm", None)
    raw = getattr(llm_cfg, "context_overflow_threshold", _OVERFLOW_THRESHOLD_DEFAULT)
    try:
        v = float(raw)
    except (TypeError, ValueError):
        return _OVERFLOW_THRESHOLD_DEFAULT
    # math.isnan check is explicit because all NaN comparisons are False.
    if math.isnan(v) or v <= 0.0 or v > 1.0:
        return _OVERFLOW_THRESHOLD_DEFAULT
    return v


# Sentinel default for the post-compaction step-trail overlay budget.
# Matches AgentConfig.request_context_prompt_max_tokens (engine/types.py:323).
_OVERLAY_MAX_TOKENS_DEFAULT = 2000


def _resolve_overlay_max_tokens(turn: Any) -> int:
    """Read [llm].request_context_prompt_max_tokens with a safe fallback.

    Out-of-range or non-numeric values silently fall back to the
    AgentConfig default; an exact 0 is also rejected (the hook treats
    0 as "unbounded" and a runaway trail would consume the request).
    """
    llm_cfg = getattr(getattr(turn, "config", None), "llm", None)
    raw = getattr(llm_cfg, "request_context_prompt_max_tokens", _OVERLAY_MAX_TOKENS_DEFAULT)
    try:
        v = int(raw)
    except (TypeError, ValueError):
        return _OVERLAY_MAX_TOKENS_DEFAULT
    if v <= 0 or v > 20_000:
        return _OVERLAY_MAX_TOKENS_DEFAULT
    return v
...
class AgentBootstrapStage:
    ...
    async def run(
        ...
        agent_config = AgentConfig(
            ...
            context_overflow_threshold=_resolve_overflow_threshold(inp.turn),
            request_context_prompt_max_tokens=_resolve_overlay_max_tokens(inp.turn),
