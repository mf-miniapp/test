...
async def idle_timeout_stream(
    ...
    The default 600s mirrors the project-wide ``agent_stream_idle_timeout_seconds``
    setting; the historical 30s was too aggressive for long-context prefill
    (see ``wrap_stream`` for the full rationale).
    """
...
def wrap_stream(
    ...
    *,
    ...
    Args:
        ...
        idle_timeout: Seconds before TimeoutError; None disables. The default
            is 600s to match ``agent_stream_idle_timeout_seconds`` in the
            gateway config — long enough to cover prefill + eval of 100K+
            token contexts. Callers that explicitly want a tighter window
            (e.g. interactive CLI) should pass a smaller value; callers that
            want no limit should pass ``None``. The previous 30s default was
            too aggressive and was observed tripping Ollama's slot handoff
            on long-running generations.
