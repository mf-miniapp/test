...
# Kept in lock-step with tools.builtin.sessions._SUBAGENT_SYSTEM_PROMPT and
# engine.runtime._SUBAGENT_TASK_PROTOCOL — all three sites source from
# ``opensquilla.agents.subagent_contract.build_subagent_system_prompt``.
# Sourcing from the leaf module (which has no transitive deps) keeps this
# step importable from very early in the boot sequence, before
# ``tools.builtin.sessions`` (which pulls in ``gateway.routing``) is ready.
_SUBAGENT_GROUNDING = build_subagent_system_prompt()
