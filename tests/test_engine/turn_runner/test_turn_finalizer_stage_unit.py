...
# ===========================================================================
# 2026-06-09 (Issue 14 — parent-side supervisor). The
# subagent_supervisor writes an audit marker that the parent
# LLM reads and acts on. The same pattern is now applied to
# the parent (hack-deep) itself: when its LLM emits a
# confirmation-question stall, the stage writes a system
# message to the parent's transcript with an explicit
# "AUTO-CONTINUE" instruction. The LLM's next turn (whether
# operator-triggered or auto-fired by the engine) will see
# this system message and be forced by the SOUL contract
# to dispatch the next sub-task WITHOUT asking the
# operator to confirm again.
# ===========================================================================


async def test_parent_stall_writes_auto_continue_system_message() -> None:
    """When the LLM emits a confirmation question
    (e.g. '需要我立即发起 W5 重试 + W4.3b 重试吗?'), the
    stage must:
      1. Rewrite the question to "Auto-continuing." form.
      2. Set auto_continue=True on the output.
      3. Append a system message to the parent's transcript
         with the explicit "you MUST continue" instruction.
    """
    from opensquilla.engine.turn_runner.turn_finalizer_stage import (
        TurnFinalizerStageOutput,
    )

    # The canonical 51ifind.com regression: the parent
    # LLM emits a table summarizing subagent outcomes +
    # a confirmation question at the end. The stage must
    # rewrite the question + write a system message.
    parent_stall_text = (
        "## 下一步行动\n"
        "| 优先级 | 行动 | 说明 |\n"
        "| --- | --- | --- |\n"
        "| W5 重试 | SSRF 路径注册 | 通过 POST 注册 SSRF 路由 |\n"
        "| W4.3b 重试 | 字面量 \\n 重新探测 | /encoder/en/doc |\n"
        "| W6 | Lateral Movement | 利用已创建 SSRF 路由 |\n"
        "\n"
        "需要我立即发起 W5 重试 + W4.3b 重试吗?"
    )
    stage, recs = _make_stage()
    inp = _make_input(final_text_parts=[parent_stall_text])
    outcome = await stage.run(inp)
    out = outcome.output

    # 1. The auto_continue flag is True.
    assert out.auto_continue is True
    # 2. The final_text has the question rewritten.
    assert "需要我立即发起" not in out.final_text
    assert "Auto-continuing" in out.final_text
    # 3. The transcript appender received TWO calls: the
    # rewritten assistant message + the system message.
    appender = recs["transcript_append"]
    assert len(appender.calls) >= 2
    # The first call is the rewritten assistant message
    # (role="assistant").
    assert appender.calls[0]["role"] == "assistant"
    assert "需要我立即发起" not in appender.calls[0]["content"]
    # The second call is the system message with the
    # explicit "AUTO-CONTINUE" instruction.
    sys_msg = appender.calls[1]
    assert sys_msg["role"] == "system"
    assert "AUTO-CONTINUE" in sys_msg["content"]
    assert "must proceed" in sys_msg["content"].lower() or (
        "you MUST" in sys_msg["content"]
    )
    assert "Do NOT ask" in sys_msg["content"]


async def test_parent_no_stall_does_not_write_system_message() -> None:
    """When the LLM does NOT emit a confirmation question,
    no system message is written. The LLM's normal output
    is appended once (as 'assistant') without any
    'AUTO-CONTINUE' injection.
    """
    stage, recs = _make_stage()
    # A clean, non-stalling assistant message.
    inp = _make_input(final_text_parts=["W4 done. Moving to W5."])
    outcome = await stage.run(inp)
    out = outcome.output

    # No stall → no system message.
    assert out.auto_continue is False
    # Only ONE transcript append: the assistant message.
    appender = recs["transcript_append"]
    assert len(appender.calls) == 1
    assert appender.calls[0]["role"] == "assistant"


async def test_parent_stall_with_marker_is_not_rewritten() -> None:
    """A confirmation question that follows a real RESULT
    MARKER is NOT a stall — the LLM is genuinely handing
    off. The rewrite must be a no-op and no system
    message should be written.
    """
    stage, recs = _make_stage()
    text = (
        "W4 evidence collected.\n"
        "\n"
        "schema: pentest-v1 | phase: evidence-collection | wave: 4/9 | deps: empty\n"
        "\n"
        "Shall I continue to W5?"
    )
    inp = _make_input(final_text_parts=[text])
    outcome = await stage.run(inp)
    out = outcome.output

    # Real marker → not a stall, no auto-continue.
    assert out.auto_continue is False
    assert out.final_text == text
    # Only one transcript append.
    appender = recs["transcript_append"]
    assert len(appender.calls) == 1
    assert appender.calls[0]["role"] == "assistant"
    assert "system" not in [c["role"] for c in appender.calls]
