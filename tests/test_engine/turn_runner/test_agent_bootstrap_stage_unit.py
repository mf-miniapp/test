...
# ---------------------------------------------------------------------------
# Tests — [llm].context_window_tokens + [llm].context_overflow_threshold
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_context_window_tokens_from_catalog_is_plumbed_into_agent_config() -> None:
    """[llm].context_window_tokens user override flows through the catalog port."""
    # Catalog returned by ModelCatalogPort with the user-overridden value.
    catalog = _ResolvedCatalog(
        max_tokens=4096,
        context_window=32_000,  # would normally be 200k for an unknown model
        capabilities=None,
    )
    stage = _make_stage(catalog=_RecordingModelCatalog(catalog=catalog))
    inp = _make_input()
    out = await stage.run(inp)
    assert out.output.agent_config.context_window_tokens == 32_000


@pytest.mark.asyncio
async def test_overflow_threshold_default_when_unset() -> None:
    """No [llm].context_overflow_threshold → AgentConfig default 0.85."""
    stage = _make_stage()
    inp = _make_input()  # _make_turn() leaves config=None → getattr fall-back
    out = await stage.run(inp)
    assert out.output.agent_config.context_overflow_threshold == 0.85


@pytest.mark.asyncio
async def test_overflow_threshold_user_value_is_plumbed() -> None:
    """[llm].context_overflow_threshold = 0.7 → AgentConfig.context_overflow_threshold = 0.7."""
    turn = _make_turn(
        config=SimpleNamespace(
            llm=SimpleNamespace(context_overflow_threshold=0.7),
        )
    )
    stage = _make_stage()
    inp = _make_input(turn=turn)
    out = await stage.run(inp)
    assert out.output.agent_config.context_overflow_threshold == 0.7


@pytest.mark.asyncio
@pytest.mark.parametrize(
    "bad_value",
    [0.0, -0.5, 1.5, 2.0, "abc", None, float("nan")],
    ids=["zero", "negative", "above_one_low", "above_one_high", "string", "none", "nan"],
)
async def test_overflow_threshold_invalid_value_falls_back(bad_value: Any) -> None:
    """Out-of-range / non-numeric threshold silently falls back to 0.85.

    Mirrors the defensive contract — a bad value would otherwise make L3
    fire on every turn (<=0) or never fire (>1.0); the stage prefers a
    sane default over panicking.
    """
    turn = _make_turn(
        config=SimpleNamespace(
            llm=SimpleNamespace(context_overflow_threshold=bad_value),
        )
    )
    stage = _make_stage()
    inp = _make_input(turn=turn)
