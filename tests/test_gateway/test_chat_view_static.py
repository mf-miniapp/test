...
# ---------------------------------------------------------------------------
# WebUI auto-retry (2026-06-10): the chat.js layer that re-issues chat.send
# on recoverable provider errors. Static checks so a future rename cannot
# silently drop the feature.
# ---------------------------------------------------------------------------


def test_chat_auto_retry_symbols_declared() -> None:
    """All new auto-retry symbols must be present in chat.js so the
    `.error` branch can call into them."""
    source = CHAT_JS.read_text(encoding="utf-8")
    for needle in (
        "_shouldAutoRetry",
        "_scheduleAutoRetry",
        "_applyAutoRetryPolicy",
        "_addAutoRetryIndicator",
        "_removeAutoRetryIndicator",
        "_cancelAutoRetry",
        "_autoRetryRounds",
        "_autoRetryMaxRounds",
        "_autoRetryOnKinds",
        "webui_auto_retry_max_rounds",
        "webui_auto_retry_on_kinds",
    ):
        assert needle in source, f"missing JS symbol: {needle}"


def test_chat_auto_retry_hooked_at_error_branch() -> None:
    """The `.error` branch in the session-event subscriber must consult
    _shouldAutoRetry before falling through to the user-visible error
    message; otherwise the operator sees a hard failure on every
    transient error and auto-retry never runs."""
    source = CHAT_JS.read_text(encoding="utf-8")
    start = source.index("} else if (event.endsWith('.error')) {")
    # Anchor the end at the unique ``_addMessage('error', _sessionErrorMessage(...))``
    # line that lives in the .error branch (whether the auto-retry
    # else wraps it or not).
    fallback_idx = source.index(
        "_addMessage('error', _sessionErrorMessage(payload))", start
    )
    end = fallback_idx + len(
        "_addMessage('error', _sessionErrorMessage(payload))"
    )
    block = source[start:end]
    assert "_shouldAutoRetry(payload)" in block
    assert "_scheduleAutoRetry(payload)" in block
    assert "_addMessage('error'" in block  # fallback path is still there


def test_chat_auto_retry_hooked_at_rpc_catch() -> None:
    """The chat.send RPC .catch() arm must also check the auto-retry
    policy, so transient transport failures that never produced a
    session.event.error still recover."""
    source = CHAT_JS.read_text(encoding="utf-8")
    # Anchor on the chat.send RPC call so we find the *right* .catch,
    # not a compaction or other handler's catch.
    anchor = "_rpc.call('chat.send', params)"
    call_start = source.index(anchor)
    # Find the .catch that follows the .then on this RPC chain.
    catch_start = source.index(".catch((err) => {", call_start)
    # End at the unique "Send failed:" fallback line that closes the
    # auto-retry decision in this catch arm.
    end = source.index("'Send failed: '", catch_start) + len("'Send failed: '") + 200
    # If the marker wasn't found (e.g. the auto-retry hook hasn't been
    # added yet), fall back to a window around the catch so the test
    # still gives a meaningful signal.
    if "Send failed" not in source[catch_start:end]:
        end = catch_start + 1500
    block = source[catch_start:end]
    assert "_shouldAutoRetry" in block
    assert "'Send failed: '" in block  # original error path still present


def test_chat_auto_retry_policy_push_call_sites() -> None:
    """_applyAutoRetryPolicy must be called at the same chokepoints as
    _applyRpcPolicy (initial render, reconnect, hello) so the browser
    picks up the policy on every state transition."""
    source = CHAT_JS.read_text(encoding="utf-8")
    rpc_count = source.count("_applyRpcPolicy(")
    auto_count = source.count("_applyAutoRetryPolicy(")
    assert rpc_count >= 3, f"expected >=3 _applyRpcPolicy callsites, got {rpc_count}"
    assert auto_count == rpc_count, (
        f"_applyAutoRetryPolicy ({auto_count}) must be called at the same "
        f"sites as _applyRpcPolicy ({rpc_count})"
    )
