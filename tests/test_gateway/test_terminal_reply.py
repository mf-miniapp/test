...
def test_sanitize_agent_error_normalizes_empty_response_for_large_input() -> None:
    # engine/agent.py:2529-2539 raises ErrorEvent(code="empty_response",
    # message="Provider returned no visible response for a large input…")
    # when the provider returns no visible content after the input
    # crossed the context limit.  This must be normalized to the standard
    # ``provider_request_too_large`` semantics so the main agent (and
    # the user) see a consistent "request is too large" reply, not a
    # raw provider dump.
    raw = (
        "Provider returned no visible response for a large input. "
        "Send the material as an attachment, summarize or shorten the "
    ...
    error_class, message = sanitize_agent_error(
        ...
        fallback_error_class="empty_response",
        fallback_error_message=raw,
    )

    assert error_class == "provider_request_too_large"
    assert "too large" in message.lower()
    assert raw not in message
    assert "empty_response" not in message


def test_build_terminal_reply_rewrites_empty_response_for_large_input() -> None:
    payload = {
        "status": "failed",
        "terminal_reason": "error",
        "error_class": "empty_response",
    ...
    assert "too large" in message.lower()
    assert "no visible response for a large input" not in message
