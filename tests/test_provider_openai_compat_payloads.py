...
def test_dashscope_thinking_with_sentinel_budget_omits_field(monkeypatch: Any) -> None:
    """Negative sentinel from `_THINKING_BUDGET_DEFAULT` must not leak as a
    literal ``thinking_budget`` value. The previous unconditional write let
    ``-1`` reach the upstream, which llama-server and DashScope both
    interpret as "unlimited" and surface as
    ...
    cfg = ChatConfig(
        thinking=True,
        thinking_budget_tokens=-1,  # _THINKING_BUDGET_DEFAULT sentinel
        model_capabilities=ModelCapabilities(
            supports_reasoning=True,
            supports_tools=True,
            reasoning_format="dashscope",
    ...
    assert captured["payload"]["enable_thinking"] is True
    assert "thinking_budget" not in captured["payload"], (
        "thinking_budget=-1 leaked to upstream; expected field to be omitted"
    )
